...
class KEPModelLocalSolver:
    def __init__(self, instance: KEPData):
        ...
        self.adjacency_matrix = [
            [0 for i in range(self.nb_vertices)] for j in range(self.nb_vertices)
        ]
        self.weights_matrix = [
            [0 for i in range(self.nb_vertices)] for j in range(self.nb_vertices)
        ]
        ...
        for i in range(len(self.instance.adjacency_matrix)):
            for j in range(len(self.instance.adjacency_matrix[i])):
                self.adjacency_matrix[i - 1][j - 1] = int(
                    instance.adjacency_matrix[i][j] != 0
                )
    ...
    def solve(
        ...
        with localsolver.LocalSolver() as ls:
            ...
            self.couples_sequences = [
                self.model.list(self.nb_vertices) for k in range(self.nb_lists)
            ]
            ...
            for n in range(self.nb_vertices):
                _adjacency_matrix_array.add_operand(
                    self.model.array(self.adjacency_matrix[n])
                )
            ...
            for k in range(self.nb_lists - 1):
                ...
                limit = (
                    self.instance.K
                    + (self.instance.L - self.instance.K) * is_altruist_sequence
                )
                self.model.constraint(c <= limit)

                # edges existance
                couples_selector = self.model.lambda_function(
                    lambda i: self.model.at(
                        _adjacency_matrix_array, sequence[i], sequence[i + 1]
                    )
                )
                self.model.constraint(
                    self.model.sum(self.model.range(0, c - 1), couples_selector)
                    >= c - 1
                )

                # return edge existance
                final_is_existing = self.model.at(
                    _adjacency_matrix_array, sequence[c - 1], sequence[0]
                )
                self.model.constraint((final_is_existing + is_altruist_sequence) == 1)
                # self.model.constraint((final_is_existing) == 1)

                # objective calculations
                weight_selector = self.model.lambda_function(
                    lambda i: self.model.at(
                        _weights_array, sequence[i], sequence[i + 1]
                    )
                )
                _sequences_weights[k] = self.model.sum(
                    self.model.range(0, c - 1), weight_selector
                ) + (1 - is_altruist_sequence) * self.model.at(
                    _weights_array, sequence[c - 1], sequence[0]
                )
